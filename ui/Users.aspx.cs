    ...
    public partial class Users : BasePage
    {
        ...
        private void Bind()
        {
            try
            {
                ...
                long countResponse = userService.GetTotalCount(tpdRange.Start, tpdRange.End, tbxSearch.Text);
                ...
                List<User> fetchResponse = userService.FetchWithPaging(tpdRange.Start, tpdRange.End, tbxSearch.Text, pgdData.PageSize, pgdData.CurrentPageIndex * pgdData.PageSize);
                pgdData.DataSource = fetchResponse;
                pgdData.DataBind();

                lblGridHeader.Text = "Users" + (pgdData.TotalRows > 0 ? " ({0})".Parameters(pgdData.TotalRows.ToString()) : string.Empty);
            }
            catch (Exception ex)
            {
